...
class LayeredNetworkGraph(object):
    ...
    def __init__(self, graphs,color=None, node_labels=None, layout=nx.spring_layout, ax=None):
    ...
    def draw_nodes(self, nodes, *args, **kwargs):
        ...
        self.ax.scatter(x, y, z,*args, **kwargs)#,s=2.88,c=self.color)#这个地方改节点大小参数# 
    ...
    def draw_plane(self, z, *args, **kwargs):
        ...
        U, V = np.meshgrid(u ,v)
        W = z * np.ones_like(U)
        self.ax.plot_surface(U, V, W, *args, **kwargs)#,cmap=plt.get_cmap('Blues_r'))#画平面，可设置颜色
    ...
    def draw(self):
        ...
        self.draw_edges(self.edges_within_layers,  color='k', alpha=0.2, linestyle='-', zorder=2,linewidth=1)
        self.draw_edges(self.edges_between_layers, color='silver', alpha=0.16, linestyle='--', zorder=2,linewidth=1)###这个地方改边的颜色

        for z in range(self.total_layers):
            self.draw_plane(z, alpha=0.2, zorder=1)
            self.draw_nodes([node for node in self.nodes if node[1]==z], s=1.68, zorder=3,c=self.color)##调用，可设置大小，颜色
...
if __name__ == '__main__':
    ...
    LayeredNetworkGraph([g,h,i], #node_labels=node_labels, 
